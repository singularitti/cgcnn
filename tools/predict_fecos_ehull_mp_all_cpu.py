...
MODEL_SOURCE = Path(
    "/work2/04996/tg842951/stampede3/run/mp_all_ehull_cpu_20260430_045246/model_best.pth.tar"
)
MODEL_LINK = DATASET_DIR / "model_best_mp_all_ehull_cpu_20260430_045246.pth.tar"
RAW_OUTPUT = DATASET_DIR / "test_results_mp_all_ehull_cpu_20260430_045246.csv"
PREDICTION_OUTPUT = DATASET_DIR / "ehull_predictions_mp_all_ehull_cpu_20260430_045246.csv"
METADATA_PATH = DATASET_DIR / "prediction_metadata_mp_all_ehull_cpu_20260430_045246.json"
    ...
    with raw_output.open(newline="") as handle:
        ...
        for row in reader:
            ...
            rows.append(
                {
                    "material_id": row[0],
                    "dummy_target_ehull_eV_per_atom": f"{float(row[1]):.16g}",
                    "predicted_ehull_eV_per_atom": f"{float(row[2]):.16g}",
                }
            )
    if not rows:
...
def write_metadata(row_count: int) -> None:
    metadata = {
        ...
        "purpose": "Apply the raw-target MP-all energy_above_hull CGCNN model to the Fe-Co-S MPRelaxSet CIFs.",
        ...
        "outputs": {
            ...
            "command": "uv run python tools/predict_fecos_ehull_mp_all_cpu.py",
            "batch_size": 256,
            "workers": 0,
            "device": "cpu",
        },
